...
### pixel像素屏驱动
1.1  结构体rt_lcd_device

rt_lcd_device结构体原型如下：
...
1.2  2个硬件接口函数
    ...
    这个是lcd设备的控制函数，通过它设置一些lcd的参数。


1.3  5个基本的绘图函数，UIENGINE的所有绘制操作都是以这5个绘图函数为基础的，是使用LCD必须提供的函数。
...
1.4  向RTT注册LCD设备，初始化并打开LCD设备

    调用rt_hw_lcd_init()函数，向RTT注册LCD设备，初始化并打开LCD设备。
...
2.4 编写5个基本的LCD操作函数，并用它们初始化rt_device_graphic_ops
...
2.5 调用rt_hw_lcd_init()向RTT注册LCD设备，初始化并打开LCD设备
...
2.6.1  在void rt_init_thread_entry(void* parameter)函数最后添加以下代码，初始化LCD。
...
2.6.2  在application.c中新建一个LCD测试线程入口函数
...
2.6.3  将下面代码添加到rt_application_init()函数最后。
